...
export default function Home() {
  ...
  const updateDevice = useCallback((event: DeviceEventMessage) => {
  ...
  useEffect(() => {
    ...
    if (user.status === UserStatus.OFFLINE && hasRedirectedFromAuth() && codeVerifier !== null) {
      ...
      connectDropbox(REDIRECT_URI, codeVerifier).then((dbc) => {
        dbc.usersGetCurrentAccount().then((dropBoxUser: DropboxResponse<users.FullAccount>) => {
          let name = dropBoxUser.result.name.display_name;
          userDispatch({ type: "DROPBOX_USER_LOGGED_IN", userName: name, dbc });
        }).catch((error) => {
  ...
  const openPinDialog = () => {
    ...
    getEncryptionKey(user.device.path).then(
      (keyPair) => {
        if (keyPair !== null) {
      ...
      <div className={styles.grid}>
        ...
        {user.status === UserStatus.ONLINE_NO_TREZOR &&
          <div className={styles.dropbox_user}>
            <Image src={"images/dropbox.svg"} alt={"signed in as dropbox user"} width={110} height={110} />
            <div className={styles.dropbox_user}>
              <span className={styles.dropbox_user}>Signed in as</span>
              <h3 className={styles.dropbox_user}><b>{user.dropboxAccountName}</b></h3>
              <span className={styles.connect_trezor}>
                <Image src={"images/connect-trezor.svg"} alt={"trezor-disconnected"} width={20} height={45} />
        ...
        {user.status === UserStatus.ONLINE_WITH_TREZOR &&
          <div className={styles.dropbox_user}>
            ...
            <div>
              ...
              <h3><b>{user.dropboxAccountName}</b></h3>
