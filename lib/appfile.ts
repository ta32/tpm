...
export async function appFileName(masterPk: string): Promise<string> {
  ...
  const cryptoKey = await crypto.subtle.importKey('raw', enc.encode(fileKey), algorithm, false, ['sign', 'verify']);
  const signature = await crypto.subtle.sign(algorithm.name, cryptoKey, enc.encode(FILENAME_MESS));
